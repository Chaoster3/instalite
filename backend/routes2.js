...
const register = async function (req, res) {
    ...
    if (username == null || password == null || firstName == null || lastName == null || email == null || affiliation == null || birthday == null) {
        return res.status(400).json({ error: 'One or more of the fields you entered was empty, please try again.' });
    }
    try {
        const existing = await db.send_sql(`SELECT * FROM users WHERE username = '${username}'`);
        ...
        await db.send_sql(`INSERT INTO users (username, password, firstName, lastName, email, affiliation, birthday) VALUES ('${username}', '${hashed}', '${firstName}', '${lastName}', '${email}', '${affiliation}', '${birthday}')`);
        const found = await db.send_sql(`SELECT user_id FROM users WHERE username = '${username}'`);
...
const login = async (req, res) => {
    ...
    try {
        const correct = await db.send_sql(`SELECT password, user_id FROM users WHERE username = '${username}'`);
        let matches;
        if (correct.length < 1) {
            matches = false;
        } else {
            matches = await new Promise((resolve, reject) => {
                bcrypt.compare(password, correct[0]['hashed_password'], (err, result) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(result);
